#!/usr/bin/env python3
"""Generate a version-stamped Atlassian CLI Confluence reference from installed --help output.

Safety invariant: this script only invokes `acli --version` and `acli ... --help`.
It never executes create/edit/delete/archive/authenticated data-changing commands.
"""
...
DEFAULT_OUTPUT = Path(__file__).with_name("ACLI-CONFLUENCE-REFERENCE.md")
NON_CONFLUENCE_GROUPS = {"admin", "auth", "jira", "guard", "rovodev", "config", "feedback"}
...
# loaded into an agent's context. The confluence group is small (auth, blog, page,
# space), so the allowlist covers the read and write commands people actually reach
# for; auth logout/switch and space archive/restore stay tree-only.
...
def discover(acli: str) -> list[CommandDoc]:
    ...
    for name in top_names:
        if name == "confluence":
            visit(("confluence",), recurse=True)
        elif name in NON_CONFLUENCE_GROUPS:
...
def render_markdown(
    ...
    confluence_count = sum(1 for doc in docs if doc.words and doc.words[0] == "confluence")
    non_confluence_count = command_count - confluence_count
    lines: list[str] = [
        ...
        f"- **Commands discovered:** `{command_count}` (`{confluence_count}` Confluence, `{non_confluence_count}` top-level/non-Confluence inventory)",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate a markdown acli Confluence reference from installed --help output.")
